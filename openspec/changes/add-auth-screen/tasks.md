...
- [ ] 6.1 Cold start with no credentials → biometric (if enrolled) → splash → `/sign-in`
- [ ] 6.2 Tap "Sign in" → system browser opens Auth0 universal login → username/password sign-in → returns to `(tabs)`
- [ ] 6.3 Tap "Sign in" → cancel system browser → screen returns to idle, no error message
- [ ] 6.4 Sign in via Google connection → returns to `(tabs)` with `user.email` populated
- [ ] 6.5 Kill and relaunch app → cold start lands directly on `(tabs)` (silent refresh path)
- [ ] 6.6 Open Profile tab → tap "Sign out" → lands on `/sign-in`; relaunch confirms credentials are cleared
- [ ] 6.7 With airplane mode enabled, tap "Sign in" → inline error appears and the button stays enabled
